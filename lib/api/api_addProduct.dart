...
class api_addProduct {
  static Future addProduct(
    ...
    for (int i = 0; i < photos.length; i++) {
      data['photos[$i]'] = photos[i];
    }
    for (int i = 0; i < videos.length; i++) {
      data['videos[$i]'] = videos[i];
    }
    ...
    final response = await http.post(
      Uri.parse(baseUrl + addProductUrl),
      headers: {
    ...
    return model_addProduct.fromJson(jsonDecode(response.body));
